...
int main()
{
	//noConflictInsertionTest();
	//conflicInsertionTest();
	//removePlayerTest();
	//moveTest();
	//printTest();
	...
	B.move(4, 9, 3);	//assumed syntax: move(ID,x,y) // removes 7 in the process
	B.printByID();

	B.move(4, 9, 8);	// removes 8
	B.printByID();

	B.move(4, 6, 3); // should fail because move to 6,3 is from 4's current position is not valid
	B.printByID();

	B.move(2, 3, 3);
	B.printByID();

	B.move(4, 3, 2);	// removes 1
	B.printByID();

	B.move(4, 8, 2);	// removes 6
	B.printByID();

	B.move(5, 3, 3);	// removes 2
	B.printByID();

	B.move(4, 4, 6);	// removes 3
	B.printByID();

	B.move(5, 4, 3);
	B.printByID();

	B.move(4, 4, 3);	// removes 5
	B.printByID();
